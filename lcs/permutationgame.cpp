...
void solve()
{
    ...
    // 0 for losing and 1 for winning..
    // if from i, A can go at a pos which is losing then that ith pos is winning for A vice versa
    ans[n] = 0;
    ...
    for (int i = n - 1; i >= 1; --i)
    {
    	...
    	for (int j = s - i; j >= 1; j -= i)
    	{
    		if (a[j] > i)
                if (ans[a[j]] == 0)
                    flag = 0;
    	}
        ans[i] = (1^flag); //if flag = 0, ans[i] = 1 vice-versa..
    }
    for (int i = 1; i < n + 1; ++i)
        (ans[a[i]] == 1) ? cout<<'A' : cout<<'B';
}
int main()
{
#ifndef ONLINE_JUDGE
